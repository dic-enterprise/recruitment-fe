...
function parseRequirementsToForm(job: Job): { description: string; skills: string[]; urgency: "NORMAL" | "URGENT" } {
  ...
  if (km && km.index !== undefined) {
    if (skills.length === 0) {
      skills = km[1].split(",").map(s => s.trim()).filter(Boolean);
    }
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            />
            {getFormErrorMessage("title") ? <p className="text-sm text-destructive">{getFormErrorMessage("title")}</p> : null}
          </div>
          <div className="space-y-2">
        ...
        <CardContent className="space-y-2">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-sm text-sky-900 dark:border-sky-900 dark:bg-sky-950/40 dark:text-sky-100">
            <div className="flex items-start gap-2">
              ...
              <span>
                <span className="font-medium">AI Insight:</span> Thêm &apos;Yêu cầu Portfolio&apos; để thu hút ứng viên chất lượng hơn.
              </span>
            </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {getFormErrorMessage("skills") ? <p className="text-sm text-destructive">{getFormErrorMessage("skills")}</p> : null}
