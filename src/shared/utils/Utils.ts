...
export class AppValidation {
  ...
  static validateField<FormType, Field extends keyof FormType>(
    value: FormType[Field],
    validators: Validator<FormType[Field]>[],
  ): [boolean, string] {
    for (let i = 0; i < validators.length; i++) {
  ...
  static getErrorValidate<FormType extends object>(
    object: FormType,
    validatorRecord: FieldValidatorRecord<FormType>,
