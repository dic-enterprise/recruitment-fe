...
import type { ExtractStatus, JobStatus, EmploymentTag, MatchQueueStatus } from "@/lib/mock-data";
...
export function ScoreBadge({ score }: { score: number }) {
  ...
  return (
    <Badge className={cn("border-0 text-sm font-bold", isHigh ? "bg-success text-success-foreground" : score >= 60 ? "bg-warning text-warning-foreground" : "bg-destructive text-destructive-foreground")}>
      {score}%
    </Badge>
  );
}
