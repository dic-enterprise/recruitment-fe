...
import { candidates, getMatchesForCandidate } from "@/lib/mock-data";
import { ExtractStatusBadge, EmploymentBadge, ScoreBadge } from "@/components/StatusBadges";
      ...
      <PageHeader
        ...
        actions={
          <Button variant="outline" asChild>
            <Link to="/hr/candidates"><ArrowLeft className="mr-2 h-4 w-4" />Back</Link>
          </Button>
        }
      />
        ...
        <div className="space-y-4 lg:col-span-1">
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              <div className="flex items-center gap-2"><Mail className="h-4 w-4 text-muted-foreground" />{candidate.email}</div>
              {candidate.phone && <div className="flex items-center gap-2"><Phone className="h-4 w-4 text-muted-foreground" />{candidate.phone}</div>}
              <div className="flex items-center gap-2"><FileText className="h-4 w-4 text-muted-foreground" />{candidate.cvFileName}</div>
              <div className="flex items-center gap-2">
                <span className="text-muted-foreground">Uploaded:</span>{candidate.uploadedAt}
              </div>
            ...
            <Card className="border-destructive">
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2 text-destructive">
                  <AlertCircle className="h-4 w-4" />Extract Error
                </CardTitle>
              </CardHeader>
              <CardContent>
          ...
          {candidate.extractStatus === "COMPLETE" && candidate.skills && (
            <Card>
              <CardHeader><CardTitle className="text-base">Extracted Info</CardTitle></CardHeader>
              <CardContent className="space-y-3 text-sm">
                {candidate.experience && <p><span className="text-muted-foreground">Experience:</span> {candidate.experience}</p>}
                <div>
                ...
                {matches.sort((a, b) => b.score - a.score).map(m => (
                  <div key={m.id} className="rounded-lg border p-4">
                    <div className="flex items-center justify-between mb-2">
                      <Link to={`/hr/jobs/${m.jobId}`} className="font-medium text-primary hover:underline">{m.jobTitle}</Link>
                      <div className="flex items-center gap-2">
