    ...
    <div>
      <PageHeader
        ...
        actions={
          <Button variant="outline" asChild>
            <Link to="/hr/departments"><ArrowLeft className="mr-2 h-4 w-4" />Back</Link>
          </Button>
        }
      />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader><CardTitle className="text-base">Jobs ({deptJobs.length})</CardTitle></CardHeader>
          <CardContent>
            {deptJobs.length === 0 ? (
              ...
              <div className="space-y-2">
                {deptJobs.map(job => (
                  <Link key={job.id} to={`/hr/jobs/${job.id}`} className="flex items-center justify-between rounded-lg border p-3 transition-colors hover:bg-muted">
                    <div>
